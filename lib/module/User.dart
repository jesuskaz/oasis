...
class User
{
  ...
  User({required int id, required String name, required String family, required String mobile, required String token})
  {
    this._id = id;
    this._name = name;
    this._family = family;
  ...
  static Future<User> authenticate(String username, String password) async{
    Map<String, dynamic> _res = await postToServer(api: 'Authenticate', body: {'mobile': username,  'password': password}, header: {});
    if (_res['msg'] == "Success")
